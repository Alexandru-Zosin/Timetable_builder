...
let teacher_schedule = {};   // which timeslots are occupied by each teacher (R6)
let group_schedule = {};     // which timeslots are occupied by each group / detect overlapping classes (to detect overlaps, R2/R7)
let room_schedule = {};      // which time slots are occupied by each room (R4.1)
        ...
        for (const subject of loaded_data.subjects) {
            if (subject.is_optional === 0) {  // mandatory subject
                for (const group of loaded_data.groups) {
                    ...
                    } else if (group.code !== 0) {  // seminar group (each individual subgroup) --- avoids EVERYONE
                        class_list.push({
                            type: 'seminar',
                            subject_code: subject.code,
                            group_code: group.code
...
function generateTimetableHillClimbing() {
     ...
     * It repeatedly attempts randomized initial schedules and then iteratively 
     * improves them by local changes.
     * Hard constraints R2-R8 are enforced or penalized (no overlapping classes for the
     *  same group or related groups (R2, R7),
     * no teacher teaching two classes at the same time (R3), room availability and one
     *  class per room (R4, R4.1),
     * room suitability for courses (R5), teacher max weekly hours (R6), and teacher 
     * eligibility (R8)).
     * Once a schedule satisfies all hard constraints, the algorithm tries to improve 
     * soft constraints:
     * limiting daily hours (E1) and avoiding unpreferred timeslots (E2).
     */
    const HARD_VIOLATION_WEIGHT = 100;
    const SOFT_VIOLATION_E1_WEIGHT = 1;
    const SOFT_VIOLATION_E2_WEIGHT = 5;
    ...
    function calculateCost(assign) {
        ...
        for (let i = 0; i < classCount; i++) {
            ...
            if (unpreferredSlots.includes(time_code)) {
                softViolationsE2 += 1;  // teacher is assigned to an unpreferred timeslot (E2)
            }
        ...
        for (const t in dailyCount) {
            ...
            for (const day in dailyCount[t]) {
                if (dailyCount[t][day] > maxDaily) {
                    softViolationsE1 += (dailyCount[t][day] - maxDaily);
                }
            }
        }
        // Total cost = weighted hard violations + soft violations
        return hardViolations * HARD_VIOLATION_WEIGHT + 
        softViolationsE1 * SOFT_VIOLATION_E1_WEIGHT + 
        softViolationsE2 * SOFT_VIOLATION_E2_WEIGHT;
    }
    ...
    while (Date.now() - startTime < timeOut) {
        ...
        // GENERATES AN INITIAL TIMETABLE (RANDOM RESTART)
        ...
        // Hill Climbing on that timetable: iteratively improve the assignment by local changes
        let improved = true;
        ...
        // Continues with another RESTART TIMETABLE until time runs out
    } 
