...
let groups, rooms, teachers, subjects, time_slots, extra_restrictions;
...
function initializeData(old_class_list) {
 ...
 /**
     ...
     */
function generateTimetableHillClimbing(constr_teacher) {
...
async function generateTimetableAndClasslist(new_extra_restrictions, teacher_id, oldTimetable) {
    ...
    startTime = Date.now();
    const success = generateTimetableHillClimbing(teacher_id);
    return success ? {
        data: best_timetable,
